...
export type { PermissionSet, UserProfile, PermissionCatalog, PermissionAction };
...
class Repository {
  ...
  static initialize(selectedUserId?: string | null): Promise<void> {
    if (this._initialized && selectedUserId === undefined) return Promise.resolve();
    if (this._initPromise && selectedUserId === undefined) return this._initPromise;
  ...
  private static applyBootstrap(data: BootstrapPayload): void {
    ...
    this.cache.brandingConfig = data.brandingConfig
      ? (data.brandingConfig as unknown as BrandingConfig)
      : null;
  ...
  static setSessionSuperAdmin(value: boolean): void {
    this._sessionSuperAdmin = value;
  }
  ...
  static setSessionUser(user: UserModel | null, isSuperAdmin = false): void {
    ...
    this._sessionSuperAdmin = isSuperAdmin;
  }

  private static async _doInitialize(selectedUserId?: string | null): Promise<void> {
    try {
      const scopeId = selectedUserId ?? this._viewAsUserId ?? undefined;
      const data = await bootstrapApi(scopeId);
  ...
  static async changeActiveUser(viewAsUserId: string | null): Promise<void> {
    ...
    const sessionUser = this.cache.currentUser;
    const isSuperAdmin = this.cache.isSuperAdmin;
    const subscriptionStatus = this.cache.subscriptionStatus;
    this._initialized = false;
    this._initPromise = null;
    await this._doInitialize(viewAsUserId ?? undefined);
    if (sessionUser) this.cache.currentUser = sessionUser;
    this.cache.isSuperAdmin = isSuperAdmin;
    if (subscriptionStatus) this.cache.subscriptionStatus = subscriptionStatus;
  }

  static async refreshFromServer(): Promise<void> {
    this._initialized = false;
    this._initPromise = null;
    await this.initialize(this._viewAsUserId);
  }
  ...
  static getBrandingConfig(): BrandingConfig {
    return this.cache.brandingConfig || { ...EMPTY_BRANDING };
  }

  static async saveBrandingConfig(
    ...
    try {
      const saved = await saveBrandingApi(updated);
      if (saved) this.cache.brandingConfig = saved as unknown as BrandingConfig;
    } catch (err) {
      console.error('[Repo] Branding save failed:', err);
    }
    return this.cache.brandingConfig || updated;
  }
  ...
  static saveUser(user: Partial<UserModel> & { id?: string; email: string; name: string }): UserModel {
    ...
    const base: UserModel = {
      ...
      subscription: user.active === false ? null : (user.subscription ?? null),
      permissions: {
        ...
        allowedPages: user.permissions?.allowedPages ?? ['Dashboard'],
        canUseSubscription: user.active === false ? false : (user.permissions?.canUseSubscription ?? false),
        canViewOthers: user.permissions?.canViewOthers ?? false,
        pagePermissions: user.permissions?.pagePermissions,
        fieldPermissions: user.permissions?.fieldPermissions,
        dataAccessScope: user.permissions?.dataAccessScope,
        resourceLimits: user.permissions?.resourceLimits,
      },
    ...
    if (idx !== -1) users[idx] = { ...users[idx], ...base, updatedAt: now };
  ...
  static deleteUser(id: string): void {
    this.cache.users = this.getUsers().filter((u) => u.id !== id);
  }
